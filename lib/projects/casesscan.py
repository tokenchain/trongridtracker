from lib.graphz.mistapi import enable_USDT_BEP20, enable_USDT_TRC20, enable_DAI_POLYGON, enable_USDT_ERC20, \
    enable_TRX_TRON
from lib.graphz.mistgraph import MistAnalysis, SlowMistAnalysisV2
...
def ppd_analysis():
    ...
    ma.setName("christmas-pdd")
    ma.lineThickness()
    ma.setThreadHoldUSD(bar_small).setEnableSideNote().startPlotDepth(3)


def gamble_case_1_analysis():
    ma = SlowMistAnalysisV2("GAMBLE_CASE_3")
    ma.acquireByFile("gameoperator.txt", True)
    ma.setName("gameop-2")
    ma.doIndependChart()
    ma.lineThickness()
    ma.setThreadHoldUSD(500).setEnableSideNote().startPlotV2SingleOneLayer()
